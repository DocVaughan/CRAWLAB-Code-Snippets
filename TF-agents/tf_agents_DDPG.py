...
# tf_agents_DDPG.py
#
# Script to train the voxel-builder environment using a DDPG agent. 
# Based on example code in the tf-agents repository, the origina header and 
# license information is included below.
#    # coding=utf-8
...
def train_eval(root_dir,
    ...
    with tf.compat.v2.summary.record_if(
        ...
        if num_parallel_environments > 1:
            ...
            tf_env = tf_py_environment.TFPyEnvironment(env_load_fn(env_name, max_episode_steps=max_ep_steps))
        eval_env_name = eval_env_name or env_name
        eval_tf_env = tf_py_environment.TFPyEnvironment(env_load_fn(eval_env_name, max_episode_steps=max_ep_steps))
        ...
        for _ in range(num_iterations):
            ...
            if global_step.numpy() % log_interval == 0:
                logging.info('step = {:d}, loss = {:f}'.format(global_step.numpy(), 
                                                                train_loss.loss))
    
                steps_per_sec = (global_step.numpy() - timed_at_step) / time_acc
                logging.info('{:.3f} steps/sec'.format(steps_per_sec))
