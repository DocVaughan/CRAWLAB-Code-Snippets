...
###############################################################################
# openCV_cameraCalibration.py
#
# Script to do basic camera calibration in OpenCV using a checkerboard. As 
# written, this code uses the 10x8 (9x7 internal) grid found at:
...
images = glob.glob('calibration/iPhone*.jpg')
...
for fname in images:
    ...
    if ret == True:
        ...
        cv2.imshow('img',img)
        cv2.waitKey(500) # Display until a key is pressed or 500ms passes
...
img = cv2.imread('calibration/iPhoneCamCalibration_1.jpg')
h,  w = img.shape[:2]
